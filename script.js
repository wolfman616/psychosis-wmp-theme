...
function currentget(media) {
		scrolltxt.value="dildo";
	if(media=="player") {
		scrolltxt.value="same";
		media=player.currentMedia.name;
	} else {
		scrolltxt.value="changed";
		media=player.currentMedia.name;
	}
}
function cuntfk() {
	scrolltxt.value="adawdawdawdawd";
		PList.visible="false";
}
function spuink(sc) {
	if(sc=="159")
...
function puls8() {
	plyhv2.alphaBlendTo(255,600);
	plyhv2.ontimer="jscript:puls8off();";
}
function puls8off() {
	plyhv2.alphaBlendTo(0, 1000);
	plyhv2.ontimer="jscript:puls8();";
}

function initfx() {
	myeffects.previous(); myeffects.currentPreset=2;
	myeffects.previous(); myeffects.currentPreset=2;
	init=true;
}

function Playtoggle(vsr) {
	if(vsr=="1") {
		vsr="3"; player.Controls.play();
	} else if(vsr=="2") {
		vsr="3"; player.Controls.play();
	} else if(vsr=="3"){
		vsr="2"; player.Controls.Pause();
	}
	ptoggle_Enact_butts(vsr);
}

function pToggle_Enact_butts(inputvsr) {
	if(init==false)
		initfx();
	 else if(init==true)
		initfx();
